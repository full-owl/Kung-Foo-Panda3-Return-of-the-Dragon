        ...
        <div class="row">
            <div class="col d-flex flex-column">
                <div>
                    ...
                    <button :disabled='isBowlDisabled' type="button" class="btn btn-primary" @click="addType(item.name, item.price)">{{ item.name }}</button>
                    </li>
                </div>
                <div>
                    <h2>Side</h2>
                    <li v-for="item in sides" :key="item.id">
                    <button :disabled='isSideDisabled' type="button" class="btn btn-primary" @click="addSide(item.name, item.id)">{{ item.name }}</button>
                    </li>
                </div>
                <div>
                    <h2>Entree</h2>
                    <li v-for="item in entrees" :key="item.id">
                    <button :disabled='isEntreeDisabled' type="button" class="btn btn-primary" @click="addEntree(item.name, item.id)">{{ item.name }}</button>
                    </li>
                </div>
                <div>
                    <h2>Appetizer</h2>
                    <li v-for="item in appetizers" :key="item.id">
                    <button :disabled='isAppDisabled' type="button" class="btn btn-primary" @click="addApp(item.name, item.id)">{{ item.name }}</button>
                    </li>
                </div>
                <div>
                    <h2>Drinks</h2>
                    <li v-for="item in drinks" :key="item.id">
                    <button :disabled='isDrinkDisabled' type="button" class="btn btn-primary" @click="addDrink(item.name, item.id)">{{ item.name }}</button>
                    </li>
                </div>
                <div class>
                    ...
                    <p>{{ OrderType }}</p>
                    <ul>
                        <li v-for="part in OrderItemNames" :key="part">
                            {{part}},
                        </li>
                    </ul>
...
export default {
    ...
    methods: {
        ...
        addOrderToReciept() {
            this.$refs.recieptTable.addToOrder(this.OrderType, this.OrderItems, this.OrderPrice, this.OrderItemNames);
            this.clearSelected();
        },
